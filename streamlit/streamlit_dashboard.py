...
st.sidebar.markdown("- **OLTP**: CSV -> Kafka -> Spark Streaming -> Postgres -> Streamlit")
st.sidebar.markdown("- **OLAP**: Kafka -> HDFS (Bronze/Silver/Gold) -> Trino -> Power BI")
...
def load_kpis(where_sql, params):
    q = text(f"""
        SELECT
          ...
          COALESCE(AVG(netrevenue), 0)::float8 AS avg_order_value,
...
def load_timeseries(where_sql, params):
    q = text(f"""
        SELECT
          ...
          COALESCE(SUM(netrevenue), 0)::float8 AS revenue
        FROM orders_live
        {where_sql}
        GROUP BY 1
        ORDER BY 1
...
kpi = kpi.iloc[0].to_dict() if not kpi.empty else {
    "total_orders": 0, "net_revenue": 0.0, "avg_order_value": 0.0, "unique_customers": 0
}
...
# ----------------------------
st.markdown("---")
st.header("Realtime Trend (minute buckets)")
...
with colA:
    if ts.empty:
        ...
        fig_rev = px.scatter(
            ...
            labels={"minute": "Time", "revenue": "Net Revenue ($)"}
        )
...
with colS1:
    st.subheader("Net revenue vs Quantity")
    ...
    if not scatter_df.empty:
        ...
        fig = px.box(
            ...
            color=color_col,
            points="all",
            title="Net revenue distribution by Quantity (box + points)",
            labels={"quantity": "Quantity", "totalamount": "Net Revenue"}
        )
...
with colS2:
    ...
    if not scatter_df.empty:
        fig = px.histogram(scatter_df, x="totalamount", nbins=40, title="Net revenue distribution")
        fig.update_layout(height=420)
        st.plotly_chart(fig, use_container_width=True)
...
if latest.empty:
    ...
    with st.expander("Download filtered latest orders"):
